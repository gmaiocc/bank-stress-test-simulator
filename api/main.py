...
app = FastAPI(title="Bank Stress Test API", version="0.2.0")
...
def _normalize_df(df: pd.DataFrame) -> pd.DataFrame:
    """Normaliza tipos e cria colunas auxiliares robustas ao schema do frontend."""
    ...
    cat_l = df["category"].str.lower()
    type_l = df["type"].str.lower()

    def infer_side(row):
        c = row["category"].lower()
        t = row["type"].lower()
        if c in {"asset", "liability", "equity"}:
            return c
        if t in {"asset", "liability", "equity"}:
            return t
        if "equity" in row["name"].lower() or t == "equity":
            return "equity"
